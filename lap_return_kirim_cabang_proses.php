...
if(isset($_POST['aksi'])){
	if($_POST['aksi']=='data'){
		...
		$a = mysql_query("select tgl_return, m.no_nota, nama_cabang, nama, d.kode, tipe, nama_barang, d.harga_modal, qty, d.harga_modal*qty sub_total from karyawan k join return_kirim_cabang m on k.username=m.username join return_kirim_cabang_detail d on m.no_nota=d.no_nota join barang b on d.kode=b.kode join cabang c on m.kode_cabang=c.kode_cabang where m.id_cabang='$id_cabang' and c.jenis='cabang' and tgl_return between '$dari' and '$sampai' order by tgl_return asc, no_nota asc");
		...
		$daris = explode("/", escape($_POST['rkcdari']));
		$dari = $daris[2].'-'.$daris[1].'-'.$daris[0];
		$sampais = explode("/", escape($_POST['rkcsampai']));
		$sampai = $sampais[2].'-'.$sampais[1].'-'.$sampais[0];	
		$a = mysql_query("select tgl_return, m.no_nota, nama_cabang, nama, d.kode, tipe, nama_barang, d.harga_modal, qty, d.harga_modal*qty sub_total from karyawan k join return_kirim_cabang m on k.username=m.username join return_kirim_cabang_detail d on m.no_nota=d.no_nota join barang b on d.kode=b.kode join cabang c on m.kode_cabang=c.kode_cabang where m.id_cabang='$id_cabang' and c.jenis='cabang' and tgl_return between '$dari' and '$sampai' order by tgl_return asc, no_nota asc");
		$jml = mysql_num_rows($a);
		if($jml==0){
			...
			require_once('lap_return_kirim_cabang_cetak.php');
		}
		...
		$sampais = explode("/", escape($_POST['rkcsampai']));
		$sampai = $sampais[2].'-'.$sampais[1].'-'.$sampais[0];
		header("location:lap_return_kirim_cabang_excel.php?dari=$dari&sampai=$sampai");
	}
