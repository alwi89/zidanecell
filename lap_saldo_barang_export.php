...
while($qHasil = mysql_fetch_array($query)){
  ...
  $data[] = 
    array("KODE" => $qHasil['kode'], "TIPE" => trim($qHasil['tipe']), 'NAMA BARANG' => $qHasil['nama_barang'], 'HARGA MODAL' => toIdr($qHasil['harga_modal']), 'SALDO' => $saldo, 'SUB TOTAL' => toIdr($qHasil['harga_modal']*$saldo), 'SALDO RUSAK' => $saldo_rusak);
}
  ...
  $filename = "laporan_saldo_barang_$_GET[nama]".date('Y-m-d').".xls";
