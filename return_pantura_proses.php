...
if(isset($_POST['aksi_rpt'])){
	if($_POST['aksi_rpt']=='tambah'){
		$no_nota = escape($_POST['no_nota_rpt']);
		$tgl_keluars = explode("/", escape($_POST['tgl_keluar_rpt']));
		...
		if($a){
			...
			foreach ($contents as $id=>$qty) {
				...
				$d = mysql_query("insert into return_kirim_cabang_detail(kode, qty, no_nota, harga_modal) values('$c[kode]', '$qty', '$no_nota', '$c[harga_modal]')");
				if($d){
					//update pengurangan saldo cabang yang mengirim return
					...
					//update penambahan saldo barang di pusat
					...
					unset($_SESSION['brpt']);
					$data[] = array('status' => 'ok',  'pesan' => 'berhasil menyimpan return pantura', 'no_nota' => $no_nota);
				}else{
		...
		echo json_encode($data);
	}else if($_POST['aksi_rpt']=='data barang'){
	...
	}else if($_POST['aksi_rpt']=='data cabang'){
...
//$_POST['kode']='12345';
//$_POST['
	$q = mysql_query("select * from barang where kode='$_POST[rptkode]'");
	$cek = mysql_num_rows($q);
	if($cek!=0){
		$kode = escape($_POST['rptkode']);
		$qty = escape($_POST['rptqty']);
		for($i=1; $i<=$qty; $i++){
			if(!isset($_SESSION['brpt'])){
				$_SESSION['brpt'] = $kode;
			}else{
